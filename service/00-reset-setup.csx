...
return await Paved.ProceedAsync(async () =>
{
    ...
    await "dotnet".args("script", ThisSource.RelativeFile("02-containers-build.csx"), "--no-pause").echo().result().success();
    await "dotnet".args("script", ThisSource.RelativeFile("03-containers-restart.csx"), "--no-pause").echo().result().success();
    await "dotnet".args("script", ThisSource.RelativeFile("21-vw-create-test-entities.csx"), "--no-pause").echo().result().success();
    await "dotnet".args("script", ThisSource.RelativeFile("31-vwac-update-config.csx"), "--no-pause").echo().result().success();
